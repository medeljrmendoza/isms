...
import type { InternalAuditDetail, InternalAuditOption, InternalAuditRow } from "./internalAudit";
import { Button } from "../../components/ui/Button";
import { Modal } from "../../components/ui/Modal";
import { InternalAuditForm } from "./InternalAuditForm";
import { InternalAuditViewModal } from "./InternalAuditViewModal";
...
export function InternalAuditsPage() {
  const [vessels, setVessels] = useState<InternalAuditOption[]>([]);
  ...
  const [editing, setEditing] = useState<InternalAuditDetail | null>(null);
  const [viewing, setViewing] = useState<InternalAuditDetail | null>(null);
  const [actionError, setActionError] = useState<string | null>(null);

  useEffect(() => {
    internalAuditService.options().then((data) => {
  ...
  useEffect(() => {
    ...
    internalAuditService
      ...
      .catch(() => {
        if (isMounted) setError("Couldn't load Internal Audits. Please try again.");
      })
      .finally(() => {
  ...
  const openView = async (id: number | string) => {
    setActionError(null);
    const detail = await internalAuditService.show(id);
    setViewing(detail);
  };
    ...
    <div className="p-6">
      <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
          <h1 className="text-base font-semibold text-slate-800">Internal Audits</h1>
          <Button
            type="button"
            variant="success"
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="border-b border-slate-100">
                  ...
                  <td className="px-2 py-1.5 text-slate-700">{row.auditor_name ?? "—"}</td>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (window.confirm(`Delete this report for ${row.vessel}?`)) {
                              ...
                              runAction(() => internalAuditService.destroy(row.id as number));
                            }
                          }}
                        >
                          Delete
      ...
      {formOpen && (
        <Modal title={editing ? `Edit Report — ${editing.vessel}` : "Add Internal Audit Report"} onClose={() => setFormOpen(false)}>
          <InternalAuditForm
            internalAudit={editing ?? undefined}
      ...
      {viewing && <InternalAuditViewModal internalAudit={viewing} onClose={() => setViewing(null)} />}
    </div>
  );
}
