...
import { kpiInternalAuditsService } from "./kpiInternalAuditsService";
...
const FILTER_LABELS: Record<KpiInternalAuditsFilter, string> = {
...
/** Ported from admin/kpi_internal/kpi_internal_v.php. */
export function KpiInternalAuditsPage() {
  const [options, setOptions] = useState<KpiInternalAuditsOptions>({ vessels: [] });
  const [filter, setFilter] = useState<KpiInternalAuditsFilter>("vessel");
  ...
  useEffect(() => {
    kpiInternalAuditsService.options().then(setOptions);
  }, []);

  useEffect(() => {
    setLoading(true);
    kpiInternalAuditsService
  ...
  const onBarClick = (row: KpiSummaryRow) => {
    ...
    if (filter === "vessel") {
      setDrillDown({
        title: `Internal Audit Reports — ${vessel.label}`,
        fetcher: (params) => kpiInternalAuditsService.reportsByVessel(vessel.id, params),
      });
    } else {
      setDrillDown({
        title: `Non Conformities — ${vessel.label}`,
        fetcher: (params) => kpiInternalAuditsService.nonConformitiesByVessel(vessel.id, params),
      });
    }
  };
    ...
    <div className="p-6">
      <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-4 py-3">
          <h1 className="text-base font-semibold text-slate-800">KPI — Internal Audits</h1>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-3 border-b border-slate-100 px-4 py-3">
          <div className="flex flex-col gap-1">
            ...
            <select
              value={filter}
              onChange={(e) => setFilter(e.target.value as KpiInternalAuditsFilter)}
        ...
        <div className="px-4 py-4">
          ...
          {!loading && !error && (
            <KpiBarChart
              title={`Internal Audits ${appliedFrom || appliedTo ? `(${appliedFrom ?? "…"} – ${appliedTo ?? "…"})` : `- ${new Date().getFullYear()}`}`}
