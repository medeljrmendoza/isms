...
// Accepts a number OR a numeric string — react-hook-form's `setValueAs` on
// `register()` isn't reliably applied for radio-button groups sharing a
// single field name (unlike <select>, where it works fine), so
// `nature_of_incident_id` in particular can still arrive as a string.
const optionalId = z
