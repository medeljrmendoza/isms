...
import type { FlagStateReportDetail, FlagStateReportOption, FlagStateReportRow } from "./flagState";
import { Button } from "../../components/ui/Button";
import { Modal } from "../../components/ui/Modal";
import { FlagStateReportForm } from "./FlagStateReportForm";
import { FlagStateReportViewModal } from "./FlagStateReportViewModal";
...
export function FlagStateReportsPage() {
  const [vessels, setVessels] = useState<FlagStateReportOption[]>([]);
  const [vesselId, setVesselId] = useState("ALL");
  const [appliedVesselId, setAppliedVesselId] = useState("ALL");

  const [rows, setRows] = useState<FlagStateReportRow[]>([]);
  ...
  const [editing, setEditing] = useState<FlagStateReportDetail | null>(null);
  const [viewing, setViewing] = useState<FlagStateReportDetail | null>(null);
  const [actionError, setActionError] = useState<string | null>(null);

  useEffect(() => {
    flagStateReportService.options().then((data) => {
  ...
  useEffect(() => {
    ...
    flagStateReportService
      ...
      .catch(() => {
        if (isMounted) setError("Couldn't load Flag State reports. Please try again.");
      })
      .finally(() => {
  ...
  const openView = async (id: number | string) => {
    setActionError(null);
    const detail = await flagStateReportService.show(id);
    setViewing(detail);
  };
    ...
    <div className="p-6">
      <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
          <h1 className="text-base font-semibold text-slate-800">Flag State</h1>
          <Button
            type="button"
            variant="success"
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="border-b border-slate-100">
                  ...
                  <td className="px-2 py-1.5 text-slate-700">{row.inspector ?? "—"}</td>
                    ...
                    <div className="flex flex-wrap gap-1">
                      ...
                      {row.can_publish && (
                        <Button
                          ...
                          onClick={() => runAction(() => flagStateReportService.publish(row.id as number))}
                        >
                      ...
                      {row.can_approve && (
                        <Button
                          ...
                          onClick={() => runAction(() => flagStateReportService.approve(row.id as number))}
                        >
                          Approve
                        </Button>
                      )}
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (window.confirm(`Delete this report for ${row.vessel}?`)) {
                              ...
                              runAction(() => flagStateReportService.destroy(row.id as number));
                            }
                          }}
                        >
                          Delete
      ...
      {formOpen && (
        <Modal title={editing ? `Edit Report — ${editing.vessel}` : "Add Flag State Report"} onClose={() => setFormOpen(false)}>
          <FlagStateReportForm
            flagStateReport={editing ?? undefined}
      ...
      {viewing && <FlagStateReportViewModal flagStateReport={viewing} onClose={() => setViewing(null)} />}
    </div>
  );
}
