...
/**
 * Mirrors MasterReviewRequest (backend) and, before that,
 * add_master_review.php's bootstrapValidator (quarter/year/description/
 ...
 * legacy too (their required-marker asterisks are commented out in the
 * form's own markup). There's no Vessel field — legacy's Add form hides
 * it entirely for SHORE records, and every record created here is
 * SHORE-added.
 */
export function buildMasterReviewSchema() {
  return z
    .object({
...
export type MasterReviewFormValues = z.infer<ReturnType<typeof buildMasterReviewSchema>>;
