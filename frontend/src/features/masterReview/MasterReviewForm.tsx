...
import { buildMasterReviewSchema, type MasterReviewFormValues } from "./masterReviewSchema";
import { masterReviewService } from "./masterReviewService";
import type { MasterReviewDetail, MasterReviewOption, MasterReviewOptions } from "./masterReview";
...
interface MasterReviewFormProps {
  review?: MasterReviewDetail;
  onSuccess: () => void;
  onCancel: () => void;
}
...
function detailToFormValues(r: MasterReviewDetail): MasterReviewFormValues {
  return {
    ...emptyValues(),
...
/** Ported from admin/master_review/add_master_review.php. */
export function MasterReviewForm({ review, onSuccess, onCancel }: MasterReviewFormProps) {
  const isCreate = !review;
  const [options, setOptions] = useState<MasterReviewOptions>(emptyOptions);
  const [documentOptions, setDocumentOptions] = useState<MasterReviewOption[]>([]);
  ...
  } = useForm<MasterReviewFormValues>({
    resolver: zodResolver(buildMasterReviewSchema()),
  ...
  useEffect(() => {
    masterReviewService.options().then(setOptions).catch(() => undefined);
  }, []);
  ...
  useEffect(() => {
    ...
    masterReviewService.documentOptions(selectedChapterId).then(setDocumentOptions).catch(() => undefined);
  }, [selectedChapterId]);

  const onSubmit = async (values: MasterReviewFormValues) => {
    setFormError(null);
    try {
      if (isCreate) {
      ...
      if (axios.isAxiosError(error) && isApiValidationError(error.response?.data)) {
        ...
        Object.entries(fieldErrors).forEach(([field, messages]) => {
          setError(field as keyof MasterReviewFormValues, { message: messages[0] });
        });
        return;
      }
