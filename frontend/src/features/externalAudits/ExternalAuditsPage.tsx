...
export function ExternalAuditsPage() {
  ...
  const [editing, setEditing] = useState<ExternalAuditDetail | null>(null);
  ...
  return (
    <div className="p-6">
                    ...
                    <div className="flex flex-wrap gap-1">
                      ...
                      {row.can_publish && (
                        <Button
                          ...
                          onClick={() => runAction(() => externalAuditService.publish(row.id as number))}
                        >
                      ...
                      {row.can_approve && (
                        <Button
                          ...
                          onClick={() => runAction(() => externalAuditService.approve(row.id as number))}
                        >
                          Approve
                        </Button>
                      )}
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (window.confirm(`Delete this report for ${row.vessel}?`)) {
                              ...
                              runAction(() => externalAuditService.destroy(row.id as number));
                            }
                          }}
                        >
                          Delete
      ...
      {formOpen && (
        <Modal title={editing ? `Edit Report — ${editing.vessel}` : "Add External Audit Report"} onClose={() => setFormOpen(false)}>
          <ExternalAuditForm
            externalAudit={editing ?? undefined}
