...
import { buildPscReportSchema, type PscReportFormValues } from "./pscReportSchema";
import { pscReportService } from "./pscReportService";
import type { PscReportDetail, PscReportOptions } from "./pscReport";
...
export function PscReportForm({ pscReport, onSuccess, onCancel }: PscReportFormProps) {
  ...
  useEffect(() => {
    // <option>/radio elements built from fetched options don't exist at
    // useForm's mount-time default-value assignment, so re-sync once
    // they've actually rendered.
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        {/* Frozen after creation, matching legacy's own edit form. */}
        <div className={!isCreate ? "pointer-events-none opacity-60" : undefined}>
          <SelectField
            label="Vessel"
            placeholder="Select vessel..."
            options={options.vessels.map((v) => ({ value: v.id, label: v.label }))}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <SelectField
          ...
          options={options.mou_authorities.map((m) => ({ value: m.id, label: m.label }))}
          error={errors.mou_id?.message}
          {...register("mou_id", { setValueAs: (v) => (v ? String(v) : null) })}
        />
