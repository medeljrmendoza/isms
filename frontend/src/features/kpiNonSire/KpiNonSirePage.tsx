...
import { kpiNonSireService } from "./kpiNonSireService";
...
const FILTER_LABELS: Record<KpiNonSireFilter, string> = {
  vessel: "Reports per Vessel",
  inspection_type: "Reports per Inspection Type",
};
...
/**
 * Ported from admin/kpi_non_sire/kpi_non_sire_v.php. Two of the four
 * legacy chart modes survive — Reports per Vessel and Reports per
 * Inspection Type. Observations per Vessel and Observations per
 * Inspection Type are dropped — no Observations module exists anywhere
 * in this migration. Inspection type has no lookup-table options list
 * (it's a free-text column, grouped by whatever distinct values are
 * present), so its drill-down uses the chart row's own label directly.
 */
export function KpiNonSirePage() {
  const [options, setOptions] = useState<KpiNonSireOptions>({ vessels: [] });
  const [filter, setFilter] = useState<KpiNonSireFilter>("vessel");
  ...
  useEffect(() => {
    kpiNonSireService.options().then(setOptions);
  }, []);
  ...
  const onBarClick = (row: KpiSummaryRow) => {
    if (filter === "vessel") {
      ...
      if (!vessel) return;
      setDrillDown({
        title: `Non-SIRE Reports — ${vessel.label}`,
        fetcher: (params) => kpiNonSireService.reportsByVessel(vessel.id, params),
      });
    } else {
      setDrillDown({
        title: `Non-SIRE Reports — ${row.label}`,
        fetcher: (params) => kpiNonSireService.reportsByInspectionType(row.label, params),
      });
    }
  };
    ...
    <div className="p-6">
      <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-4 py-3">
          <h1 className="text-base font-semibold text-slate-800">KPI — Non-SIRE</h1>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-3 border-b border-slate-100 px-4 py-3">
          <div className="flex flex-col gap-1">
            ...
            <select
              value={filter}
              onChange={(e) => setFilter(e.target.value as KpiNonSireFilter)}
        ...
        <div className="px-4 py-4">
          ...
          {!loading && !error && (
            <KpiBarChart
              title={`Non-SIRE ${appliedFrom || appliedTo ? `(${appliedFrom ?? "…"} – ${appliedTo ?? "…"})` : `- ${new Date().getFullYear()}`}`}
              yAxisLabel="Reports"
              rows={rows}
