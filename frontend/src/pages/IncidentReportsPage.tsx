...
import { incidentReportService } from "../api/incidentReportService";
import type { IncidentReportDetail, IncidentReportOption, IncidentReportRow } from "../types/incidentReport";
import { Button } from "../components/ui/Button";
import { Modal } from "../components/ui/Modal";
import { IncidentReportForm } from "../components/incidentReports/IncidentReportForm";
import { IncidentReportViewModal } from "../components/incidentReports/IncidentReportViewModal";
                    ...
                    <div className="flex flex-wrap gap-1">
                      ...
                      {row.can_publish && (
                        <Button
                          ...
                          onClick={() => runAction(() => incidentReportService.publish(row.id))}
                        >
                      ...
                      {row.can_approve && (
                        <Button
                          ...
                          variant="success"
                          className="!px-1.5 !py-0.5 text-xs"
                          onClick={() => runAction(() => incidentReportService.approve(row.id))}
                        >
                          Approve
                        </Button>
                      )}
                      {row.can_reopen && (
                        <Button
                          ...
                          variant="secondary"
                          className="!px-1.5 !py-0.5 text-xs"
                          onClick={() => runAction(() => incidentReportService.reopen(row.id))}
                        >
                          Re-open
                        </Button>
                      )}
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (window.confirm(`Delete this report for ${row.vessel}?`)) {
                              runAction(() => incidentReportService.destroy(row.id));
                            }
                          }}
                        >
                          Delete
